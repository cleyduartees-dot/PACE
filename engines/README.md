...
Implemented: [`project_creator.py`](./project_creator.py) — `init_instance()` (attaches a valid, structurally minimal `.pace/` to an existing project; writes placeholder mission/vision/roadmap/sprint content so `ACTIVE_VERSIONS.pdl` always points at something real) and `create_project()` (generates a brand-new project from scratch: fresh directory, `git init`, a minimal README, and a `.pace/` instance — deliberately no stack-specific scaffolding, that's a separate, later `templates/` capability). Tested in [`tests/`](./tests/) — run `python engines/tests/test_project_creator.py`.
